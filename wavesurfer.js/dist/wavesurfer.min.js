...
var WaveSurfer = {
    defaultParams: {
        ...
        var a = this, b = window.requestAnimationFrame || window.webkitRequestAnimationFrame, c = function () {
            a.backend.isPaused() || (a.drawer.progress(a.backend.getPlayedPercents()), b(c))
        };
        c()
        ...
        this.backend.play(a, b), this.restartAnimationLoop(), this.fireEvent("play")
    }, pause: function () {
...
WaveSurfer.create = function (a) {
    ...
    setFilters: function (a) {
        this.disconnectFilters(), a && a.length ? (this.filters = a, a.reduce(function (a, b) {
            return a.connect(b), b
        }, this.analyser).connect(this.gainNode)) : this.analyser.connect(this.gainNode)
    },
    createScriptNode: function () {
        var a = this, b = this.scriptBufferSize;
        this.scriptNode = this.ac.createScriptProcessor ? this.ac.createScriptProcessor(b) : this.ac.createJavaScriptNode(b), this.scriptNode.connect(this.ac.destination), this.scriptNode.onaudioprocess = function () {
            var b = a.getCurrentTime();
            a.buffer && b > a.getDuration() ? a.setState(a.FINISHED_STATE) : a.buffer && b >= a.scheduledPause ? a.setState(a.PAUSED_STATE) : a.state === a.states[a.PLAYING_STATE] && a.fireEvent("audioprocess", b)
        }
    },
    ...
    getPeaks: function (a) {
        for (var b = this.buffer.length / a, c = ~~(b / 10) || 1, d = this.buffer.numberOfChannels, e = [], f = [], g = 0; d > g; g++)for (var h = e[g] = [], i = this.buffer.getChannelData(g), j = 0; a > j; j++) {
            for (var k = ~~(j * b), l = ~~(k + b), m = 0, n = k; l > n; n += c) {
                var o = i[n];
                o > m ? m = o : -o > m && (m = -o)
            }
            h[j] = m, (0 == g || m > f[j]) && (f[j] = m)
        }
    ...
    play: function (a, b) {
        ...
        a = c.start, b = c.end, this.scheduledPause = b, this.source.start(0, a, b - a), this.setState(this.PLAYING_STATE)
    },
    pause: function () {
        this.scheduledPause = null, this.startPosition += this.getPlayedTime(), this.source && this.source.stop(0), this.setState(this.PAUSED_STATE)
    },
    getCurrentTime: function () {
    ...
    init: function (a) {
        ...
        this.seekTo(a), this.media.play(), b && this.setPlayEnd(b)
    }, pause: function () {
        this.media.pause(), this.clearPlayEnd()
    ...
    init: function (a, b) {
        ...
        this.resetScroll(), this.setWidth(b), this.drawWave(a)
    }, style: function (a, b) {
    ...
    createElements: function () {
        ...
        this.waveCc.canvas.width = this.width, this.waveCc.canvas.height = this.height, this.style(this.waveCc.canvas, {width: a + "px"}), this.progressCc && (this.progressCc.canvas.width = this.width, this.progressCc.canvas.height = this.height, this.style(this.progressCc.canvas, {width: a + "px"})), this.clearWave()
    }, clearWave: function () {
        ...
        var d = .5 / this.params.pixelRatio, e = this.params.height * this.params.pixelRatio, f = e * b || 0, g = e / 2, h = a.length, i = 1;
        this.params.fillParent && this.width != h && (i = this.width / h);
        var j = 1;
        this.params.normalize && (j = Math.max.apply(Math, a)), this.waveCc.fillStyle = this.params.waveColor, this.progressCc && (this.progressCc.fillStyle = this.params.progressColor), [this.waveCc, this.progressCc].forEach(function (b) {
            if (b) {
                b.beginPath(), b.moveTo(d, g + f);
                for (var c = 0; h > c; c++) {
                    var e = Math.round(a[c] / j * g);
                    b.lineTo(c * i + d, g + e + f)
                }
                b.lineTo(this.width + d, g + f), b.moveTo(d, g + f);
                for (var c = 0; h > c; c++) {
                    var e = Math.round(a[c] / j * g);
                    b.lineTo(c * i + d, g - e + f)
                }
                b.lineTo(this.width + d, g + f), b.closePath(), b.fill(), b.fillRect(0, g + f - d, this.width, d)
            }
        }, this)
